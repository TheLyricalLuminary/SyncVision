// Figma-ish design canvas wrapper.
// Warm gray grid bg + Sections + Artboards + PostIt notes.
// Artboards are reorderable (grip-drag), deletable, labels/titles are
// inline-editable, and any artboard can be opened in a fullscreen focus
// overlay (←/→/Esc). State persists to a .design-canvas.state.json sidecar
// via the host bridge. No external deps beyond React.
//
// Usage:
//   <DesignCanvas>
...
if (typeof document !== 'undefined' && !document.getElementById('dc-styles')) {
  ...
  s.textContent = [
    ...
    // hover-only so a narrow header is clean until the card is moused.
    ...
    // Slot hosting an open menu floats above later siblings so the popup
    // isn't clipped by the next card.
...
type CanvasState = {
  sections: Record<string, SectionPersistedState>;
  focus: string | null;
};
...
type DCContextValue = {
  state: CanvasState;
  section: (id: string) => SectionPersistedState;
  patchSection: (id: string, patch: Partial<SectionPersistedState> | ((prev: SectionPersistedState) => Partial<SectionPersistedState>)) => void;
...
// .design-canvas.state.json sidecar next to the HTML.
// ─────────────────────────────────────────────────────────────
...
export function DesignCanvas({ children, minScale, maxScale, style }: DesignCanvasProps) {
  ...
  React.useEffect(() => {
    ...
    fetch('./' + DC_STATE_FILE)
      ...
      .then((saved: { sections?: Record<string, SectionPersistedState> } | null) => {
    ...
    const t = window.setTimeout(() => { if (!off) setReady(true); }, 150);
  ...
  const api = React.useMemo<DCContextValue>(() => ({
    state,
...
function DCViewport({ children, minScale = 0.1, maxScale = 8, style = {} }: DCViewportProps) {
  ...
  const lastPostedScale = React.useRef<number | undefined>(undefined);
  ...
  React.useLayoutEffect(() => {
    ...
    try {
      const s = JSON.parse(localStorage.getItem(tfKey) || 'null') as { x: number; y: number; scale: number } | null;
  ...
  React.useEffect(() => {
    ...
    // Safari native gesture events for trackpad pinch.
    let gsBase = 1;
    let isGesturing = false;
    const onGestureStart = (e: Event) => { e.preventDefault(); isGesturing = true; gsBase = tf.current.scale; };
    const onGestureChange = (e: Event) => {
      e.preventDefault();
      const ge = e as Event & { clientX: number; clientY: number; scale: number };
...
// ─────────────────────────────────────────────────────────────
// dcExport — per-artboard PNG or self-contained HTML export
// ─────────────────────────────────────────────────────────────
  ...
  const walk = (rules: CSSRuleList, base: string) => {
    for (const r of Array.from(rules)) {
      if (r.type === CSSRule.FONT_FACE_RULE) fontRules.push({ css: r.cssText, base });
        ...
        try { if (ir.styleSheet) walk(ir.styleSheet.cssRules, ibase); } catch { scrapeCss(ibase); }
      } else if ((r as CSSGroupingRule).cssRules) walk((r as CSSGroupingRule).cssRules, base);
    }
  };
    ...
    if (src.nodeType === 1) {
      ...
      if ((src as Element).tagName === 'CANVAS') {
        try {
          ...
          im.src = (src as HTMLCanvasElement).toDataURL();
    ...
    while ((m = re.exec(bg))) {
      ...
      jobs.push(toDataURL(url).then((d) => {
        (el as HTMLElement).style.backgroundImage =
          (el as HTMLElement).style.backgroundImage.split(tok).join('url("' + d + '")');
      }));
    }
  });
...
// ─────────────────────────────────────────────────────────────
// DCArtboardFrame — the actual rendered slot for each artboard
// ─────────────────────────────────────────────────────────────
type DCArtboardFrameProps = {
  sectionId: string;
  artboard: React.ReactElement<DCArtboardProps>;
  label?: string;
  order: string[];
...
// ─────────────────────────────────────────────────────────────
// DCFocusOverlay — fullscreen focus mode; ←/→ within section,
// ↑/↓ across sections, Esc or backdrop click to exit.
// ─────────────────────────────────────────────────────────────
type DCFocusOverlayProps = {
  entry: RegistryEntry;
      ...
      >
        <div style={{ position: 'relative' }}>
          ...
          >
            ...
            {meta.subtitle && (
              <span style={{ display: 'block', fontSize: 13, opacity: .6, fontWeight: 400, marginTop: 2 }}>{meta.subtitle}</span>
            )}
          </button>
          {ddOpen && (
              ...
              {sectionOrder.filter((sid) => sectionMeta[sid].slotIds.length).map((sid) => (
                <button
                  ...
                  style={{
                    ...
                    color: '#fff', padding: '8px 12px', borderRadius: 5,
                    fontSize: 14, fontWeight: sid === sectionId ? 600 : 400, fontFamily: 'inherit',
                  }}
                >
